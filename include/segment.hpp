...
public:
    ...
    PT len;
    segment(){}
    segment(KT p1,KT p2,FT p3,FT p4,PT p5,PT p6):start(p1),end(p2),high_slope(p3),low_slope(p4),pos(p5),len(p6){}
};
template<class KT=uint64_t,class PT=size_t,class FT=double>
struct ssegment{
...
std::vector<segment<KT,PT>> shringkingCone(std::vector<KT>& keys,std::vector<PT>& pos,PT epsilon=32){
    ...
    std::vector<segment<KT,PT>> all_segs{{keys[0],keys[0],0,0,pos[0],1}};
    ...
    for(auto i=1;i<keys.size();++i){
        ...
        if(low_slope<=tmp_slope && high_slope>=tmp_slope){
            ...
            all_segs.back().high_slope=high_slope;
            all_segs.back().low_slope=low_slope;

            //开始一个新的段
            all_segs.emplace_back(keys[i],keys[i],0,0,pos[i],1);

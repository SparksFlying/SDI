...
vector<vector<uint32_t>> read_from_IMIS_txt(const string& path,uint64_t size=10) {
    std::ifstream inFile(path, std::ios::in);
	string lineStr;
	vector<vector<uint32_t>> data(size,vector<uint32_t>(2));
	uint64_t i = 0;
...
vector<vector<uint32_t>> read_from_roadnet_txt(uint64_t size=2000,int dim=2,const string& path) {
    std::ifstream inFile(path, std::ios::in);
	string lineStr;
...
vector<vector<uint32_t>> read_from_foursquare_txt(uint64_t size,const vector<size_t>& chose_which_dims,const string& path) {
    std::ifstream inFile(path, std::ios::in);
	string lineStr;
...
vector<vector<uint32_t>> read_from_US_txt(const vector<size_t>& chose_which_dims,uint64_t size=2000,const string& path) {
    std::ifstream inFile(path, std::ios::in);
	string lineStr;
	vector<vector<uint32_t>> data(size,vector<uint32_t>(chose_which_dims.size()));
	uint64_t i = 0;
...
vector<vector<uint32_t>> read_from_syn_txt(uint64_t size=2000,int dim=2,const string& path) {
    std::ifstream inFile(path, std::ios::in);
	string lineStr;
	vector<vector<uint32_t>> data(size,vector<uint32_t>(dim));
	uint64_t i = 0;
	...
	inFile.close();
    vector<vector<uint32_t>> data_unique;
	if(dim==2){
		std::unordered_set<string> st;
		for(size_t i=0;i<data.size();++i){
			if(st.find(std::to_string(data[i][0])+std::to_string(data[i][1]))==st.end()){
				st.insert(std::to_string(data[i][0])+std::to_string(data[i][1]));
				data_unique.push_back(std::move(data[i]));
			}
		}
	}
	return data_unique;
